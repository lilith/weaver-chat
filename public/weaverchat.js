    ...
    return this.each(function() {
      ...
      var appendMsg = function(msg){
        ...
        _.each(inbox, function(e){
          if (!e.html) {
            e.html = $('<li>').append(config.message_to_html(e,config));
          }
        });
      ...
      var deleteMsg = function (ids){
        idmatch = function(e){ return _.contains(ids,e.id);};
        var deleted = _.filter(inbox,idmatch);
        inbox = _.reject(inbox, idmatch);
        appendMsg([]);
      ...
      var removeFromOutbox = function(send_id){
        by_send_id = function(e){ return e.send_id === send_id;};
        var to_remove = _.filter(outbox, by_send_id);
        outbox = _.reject(outbox, by_send_id);
        updateOutboxUi();
        return to_remove;
      };
      ...
      var sendMsg = function(msg){
        msg.send_id = uuid();
        msg.send_date = (new Date()).toISOString();
        outbox.push(msg);
      ...
      //Try to authenticate
      var authinfo = JSON.stringify({authstring: config.authstring, hmacsha256: config.hmacsha256});
